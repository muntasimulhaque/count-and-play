...
1. **No music.** No melodies, tunes, rhythmic beats or instrumental loops. The
   synthesized effects in `res/raw` (regenerated by the `:tools` Kotlin generators) are
...
:tools:makeSounds                       # regenerate sound assets (byte-identical)
:tools:makeArt                          # regenerate store art + launcher icons
:tools:checkSounds                      # verify committed sound assets match regeneration
```
...
Paste-ready listing fields (short description, full description) live in
`play-store/play-store-submission-guide.md` and
`play-store/listing-full-description.txt`; store art and launcher icons are
regenerated by `:tools:makeArt`. After each release, refresh the committed
reference copies at `play-store/screenshots/` from that release's artifacts,
so the repo always shows what actually ships.
